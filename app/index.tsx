...
import { View, Text, TextInput, TouchableOpacity, KeyboardAvoidingView, Platform, ScrollView, Image, StatusBar, ActivityIndicator } from "react-native";
...
export default function LoginScreen() {
  ...
  const [isLoading, setIsLoading] = useState(true); // 💡 State untuk ngecek session di awal
  ...
  useEffect(() => {
    // 💡 SIMULASI CEK LOGIN CEK STATUS: 
    // Kalau nanti pakai AsyncStorage / SecureStore, cek tokennya di sini.
    const checkLoginStatus = async () => {
      const userIsAlreadyLoggedIn = false; // Ganti true kalau mau auto-login langsung masuk home
      
      if (userIsAlreadyLoggedIn) {
        router.replace("/(tabs)");
      } else {
        setIsLoading(false); // Tetap di halaman login jika belum masuk
      }
    };
  ...
  const handleLogin = () => {
    ...
    // 💡 Pakai replace biar user gak bisa mencet tombol 'Back' HP buat balik ke halaman login lagi
    router.replace("/(tabs)"); 
  };

  // Tampilkan loading screen sebentar pas aplikasi pertama nge-check session
  if (isLoading) {
    return (
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "center", paddingHorizontal: 28, paddingTop: 60 }}>
        ...
        <View style={{ alignItems: "center", marginBottom: 40 }}>
          ...
          <View style={{
            ...
            <Image 
              // 💡 FIX PATH: Karena file ini ditaruh di root 'app/', jalur naiknya cuma sekali '../assets'
